...
<section class="content" id="get_info" data-rid="503" data-sort="" data-kw="" data-page="">
    ...
    <div class="panel-group" id="accordion">
        <div class="panel panel-default">
            ...
            <div id="collapse1" class="panel-collapse collapse in" style="background-color:pink">
            <!-- 测试 -->
        		...
        		<?php if(is_array($zuopins)): $i = 0; $__LIST__ = $zuopins;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i;?><li class="picCon">
                        <div>
                            ...
                            <div class="clearfix">
                                ...
                                <a href="" class="vote" data-itid="<?php echo ($li["id"]); ?>" data-vote_num="<?php echo ($li["id"]); ?>" data-rule_id="<?php echo ($li["id"]); ?>">投票</a>
                                <input type="checkbox" name="check_list[]" value="<?php echo ($li["id"]); ?>"><label><?php echo ($li["item"]); ?></label><br/>

                            </div>
                        </div>
                    </li><?php endforeach; endif; else: echo "" ;endif; ?>
        </ul>
        <input class="clearfix" type="submit" name="submit" value="Submit"/>

        </form>
    </div>

    <?php
 if(isset($_POST['submit'])){ if(!empty($_POST['check_list'])){ foreach ($_POST['check_list'] as $selected) { $item = M('Vote_item')->where("id=".$selected)->setInc('vcount'); } } header("Location: http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]"); exit; } ?>
